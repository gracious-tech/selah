...
self._inobounce = inobounce.isEnabled()
...
if (process.env.NODE_ENV !== 'development' && 'serviceWorker' in self.navigator){
    // Do after everything else downloaded so doesn't interfere with initial load
    ...
    self.addEventListener('load', () => {
        ...
        // WARN Any changes to SW logic here will take place AFTER next update
        //      ie The old code will still get executed one more time before actually being updated
        // Whenever SW that is controlling page changes, reload to get updated assets
        // SW will skip waiting, so will take over from old SW and trigger this event
        // NOTE This event is NOT triggered for first SW (based on manual testing)
        self.navigator.serviceWorker.addEventListener('controllerchange', () => {
            // TODO Consider changing to a prompt when more mature
...
get_store().then(store => {
    ...
    self._toggle_dark = () => self._store.dispatch('set_dark', !self._store.state.dark)
    ...
    log_resp.then(() => {
        ...
        store.commit('dict_set', ['offline_opens', 0])
    })
    log_resp.catch(() => {
        // Offline, so increment count
        // NOTE Not chained off `then` so doesn't catch any issue with `store.commit`
        store.commit('dict_set', ['offline_opens', store.state.offline_opens + 1])
    })
})
